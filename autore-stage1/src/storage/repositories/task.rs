...
fn db_err(e: rusqlite::Error) -> autore_core::Error {
    autore_core::Error::Database(e.to_string())
}

fn json_err(e: serde_json::Error) -> autore_core::Error {
    autore_core::Error::Database(format!("JSON serialization error: {e}"))
}
...
impl TaskRepository for SqliteTaskRepository {
    ...
    async fn renew_lease(&self, task_id: TaskId, until: OffsetDateTime) -> autore_core::Result<()> {
        let conn = self.database.connection()?;
        ...
        let expires_str: String = conn
            ...
            .map_err(|e| match e {
                rusqlite::Error::QueryReturnedNoRows => {
                    autore_core::Error::Validation(format!("no active lease for task {task_id}"))
                }
                other => db_err(other),
            })?;

        let expires_ts: i64 = expires_str.parse().map_err(|_| {
            autore_core::Error::Database(format!("invalid expires_at value: {expires_str}"))
        })?;

        if expires_ts <= now_ts {
            return Err(autore_core::Error::Validation(format!(
